<?php
/**
 * Autoloader for BeauCore classes.
 *
 * @author     BeauAgency
 * @copyright  (c) Copyright by BeauAgency
 * @link       http://beau.vn
 * @package    Getypo
 * @subpackage Core
 * @since 1.0
...
/**
 * The Autoloader class for GetypoCore.
 */
class BeauCore_Autoload {
    ...
    public function __construct() {
        ...
        if ( empty( self::$transient_name ) ) {
            self::$transient_name = 'beaucore_autoloader_paths_' . md5( __FILE__ );
        }
    ...
    protected function get_path( $class_name ) {
        ...
        $paths = array();
        if ( 0 === stripos( $class_name, 'BeauCore') ) {

            $path     = BEAU_CORE_PATH . '/includes/';
            $filename = 'class-' . strtolower( str_replace( '_', '-', $class_name ) ) . '.php';

            $paths[] = $path . $filename;

            $substr   = str_replace( array( 'BeauCore_'), '', $class_name );
