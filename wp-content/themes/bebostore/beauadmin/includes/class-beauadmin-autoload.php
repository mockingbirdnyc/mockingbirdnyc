...
/**
 * The Autoloader class for BeauAdmin.
 */
class BeauAdmin_Autoload {
    ...
    public function __construct() {
        ...
        if ( empty( self::$transient_name ) ) {
            self::$transient_name = 'beauadmin_autoloader_paths_' . md5( __FILE__ );
        }
    ...
    protected function get_path( $class_name ) {
        ...
        $paths = array();
        if ( 0 === stripos( $class_name, 'BeauAdmin' ) ) {

            $path     = BeauAdmin::$template_dir_path . '/includes/';
            $filename = 'class-' . strtolower( str_replace( '_', '-', $class_name ) ) . '.php';

            $paths[] = $path . $filename;

            $substr   = str_replace( array( 'BeauAdmin_'), '', $class_name );
