			...
			<?php do_action( 'yith_wcwl_before_wishlist_form', $wishlist_meta ); ?>
				...
				<form id="yith-wcwl-form" action="<?php echo esc_url( YITH_WCWL()->get_wishlist_url( 'view' . ( $wishlist_meta['is_default'] != 1 ? '/' . $wishlist_meta['wishlist_token'] : '' ) ) ) ?>" method="post" class="woocommerce">
				    ...
				    <table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo ( is_user_logged_in() ) ? esc_attr( $wishlist_meta['ID'] ) : '' ?>" data-token="<?php echo ( ! empty( $wishlist_meta['wishlist_token'] ) && is_user_logged_in() ) ? esc_attr( $wishlist_meta['wishlist_token'] ) : '' ?>">
				        ...
				        <tr>
				            ...
				            <?php if( $show_price ) : ?>
				                ...
				                <th class="product-price col-sm-2 col-md-2 col-xs-2">
				                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price', 'bebostore' ) ) ?>
				                </th>
				            ...
				            <?php if( $show_stock_status ) : ?>
				                ...
				                <th class="product-stock-stauts col-sm-2 col-md-2 col-xs-2">
				                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status', 'bebostore' ) ) ?>
				                </th>
				            ...
				            <?php if( $show_last_column ) : ?>
				                ...
				                <th class="product-add-to-cart col-sm-2 col-md-2 col-xs-2"></th>
				        ...
				        if( count( $wishlist_items ) > 0 ) :
				            foreach( $wishlist_items as $item ) :
				                ...
				                if( $product !== false && $product->exists() ) :
				                    ...
				                    <tr id="yith-wcwl-row-<?php print ($item['prod_id']); ?>" data-row-id="<?php print ($item['prod_id']); ?>">
				                        ...
				                        <?php if( $show_stock_status ) : ?>
				                            <td class="product-stock-status col-sm-2 col-md-2 col-xs-2">
				                                <?php
				                                if( $stock_status == 'out-of-stock' ) {
				                                    $stock_status = "Out";
				                                    echo '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'bebostore' ) . '</span>';
				                                } else {
				                                    $stock_status = "In";
				                                    echo '<span class="wishlist-in-stock">' . __( 'In Stock', 'bebostore' ) . '</span>';
				                                }
				                                ?>
				                            </td>
				                        <?php endif ?>
					                    ...
					                    <?php if( $show_last_column ): ?>
				                        <td class="product-add-to-cart col-sm-2 col-md-2 col-xs-2">
					                        <!-- Date added -->
					                        <?php
					                        if( $show_dateadded && isset( $item['dateadded'] ) ):
												echo '<span class="dateadded">' . sprintf( __( 'Added on : %s', 'bebostore' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
					                        endif;
					                        ?>
					                        ...
					                        <select class="change-wishlist selectBox">
						                        <option value=""><?php _e( 'Move', 'bebostore' ) ?></option>
						                        <?php
						                        ...
						                        ?>
							                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
								                        ...
								                        if( $wl['wishlist_privacy'] == 1 ){
									                        $wl_privacy = __( 'Shared', 'bebostore' );
								                        }
								                        elseif( $wl['wishlist_privacy'] == 2 ){
									                        $wl_privacy = __( 'Private', 'bebostore' );
								                        }
								                        else{
									                        $wl_privacy = __( 'Public', 'bebostore' );
								                        }
				            ...
				            <tr>
				                <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'bebostore' ) ?></td>
				            </tr>
				        <?php
				        endif;
				        ...
				        if( ! empty( $page_links ) ) : ?>
				            <tr class="pagination-row">
				                <td colspan="<?php echo esc_attr( $column_count ) ?>"><?php print ($page_links); ?></td>
				            </tr>
				        <?php endif ?>
				        </tbody>
					        ...
					        <td colspan="<?php echo esc_attr( $column_count ) ?>">
					            <?php if( $show_cb ) : ?>
						            <div class="custom-add-to-cart-button-cotaniner">
						                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token'] ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', __( 'Add the selected products to the cart', 'bebostore' ) ) ?></a>
						            </div>
					            <?php endif; ?>
					            ...
					            <?php if ( is_user_logged_in() && $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
						            <div class="ask-an-estimate-button-container">
					                    ...
					                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate', 'bebostore' ) ) ?>
					                </a>
						            </div>
					            <?php endif; ?>
				    ...
				    <?php if( $wishlist_meta['is_default'] != 1 ): ?>
				        <input type="hidden" value="<?php print ($wishlist_meta['wishlist_token']); ?>" name="wishlist_id" id="wishlist_id">
				    <?php endif; ?>
				...
				<?php if( $additional_info ): ?>
					<div id="ask_an_estimate_popup">
						<form action="<?php print ($ask_estimate_url); ?>" method="post" class="wishlist-ask-an-estimate-popup">
							...
							<button class="btn button ask-an-estimate-button ask-an-estimate-button-popup" >
								<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_icon', '<i class="fa fa-shopping-cart"></i>' )?>
								<?php _e( 'Ask for an estimate', 'bebostore' ) ?>
							</button>
						</form>
					</div>
				<?php endif; ?>
